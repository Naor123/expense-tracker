...
def test_bank_transaction_matching_card_company_name_is_credit_card_payment_regardless_of_settlement():
    # On the real Hapoalim feed this bulk line's value_date always equals its
    # booking_date (settlement is always classified 'immediate'), even though
    # the real-world cycle settles around the 10th -- so settlement carries no
    # signal here, and a name-fragment match on a bank account's feed is
    # unambiguously the bulk lump sum either way.
    assert classify_transaction("מסטרקרד", None, "hapoalim") == "credit_card_payment"
    assert classify_transaction("MAX", None, "hapoalim") == "credit_card_payment"
